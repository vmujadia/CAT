...
Class User_Authentication extends CI_Controller {
    ...
    public function user_login_process() 
    {
        ...
        if ($this->form_validation->run() == FALSE) 
        {
		    ...
		    $modules['module_files_august'] = $this->dir_to_array(APPPATH . 'views/data/'.$this->session->userdata['logged_in']['username'].'/august/');
		    $sortfile=array();
		    $tarray=array();
		    foreach ($modules['module_files_august'] as $module_file)
		    {
		    ...
		    $modules['module_files_august']=$tarray;


		    $modules['module_files_september'] = $this->dir_to_array(APPPATH .  'views/data/'.$this->session->userdata['logged_in']['username'].'/september/');
		    $sortfile=array();
		    $tarray=array();
		    foreach ($modules['module_files_september'] as $module_file)
		    {
			$sortfile[date("F d Y H:i:s.", fileatime($module_file)).$module_file]=$module_file;
			//print date("F d Y H:i:s.", fileatime($module_file))."</br>";
		    }
		    ...
		    $modules['module_files_september']=$tarray;


		    $this->load->view('welcome_message',$modules);
		    #$this->load->view('expvandan');
	...
	public function index_part($filename)
	{
				...
				foreach ($chaindetail as $name => $locations) 
				{
					...
					$ans=$ans."<div class='span2' id='".$name."'><h5>".$name."</h5>".$a."</br><button class='btn' id='chainadd".$name."'>AddIn ".$name."</button><button class='btn' id='chainhide".$name."'>H".$name."</button></div>";
					if ($count%5==0)
                                        {
				...
				#$filename=str_replace("/","",$filename);
				$filename=str_replace(".","",$filename);
                                $filename_n = explode("/", $filename);
                                $data['filename']="".$filename_n[sizeof($filename_n)-1]."";
				return $data;
	}
	...
	public function show_chain()
	{
		   ...
		   $sendchaindetail=array();		
				...
				foreach ($chaindetail as $name => $locations) 
				{
					...
					$a="<div id='intxt".$name."' >".$a."</div>";
					$ans=$ans."<div class='span2' id='".$name."'><h5>".$name."</h5>".$a."</br><button class='btn' id='makechainhead".$name."'><span id='makechainhead".$name."A'> Sel./Rep. Head of ".$name." chain</span></button></div>";
	...
	public function savefilefinal()
        {
		   ...
		   $filename=str_replace(".","",$filename);
				...
				//fwrite($fp,$content);
				//fclose($fp);
				$filename=get_cookie('filename1_file');
				$filename=str_replace("application","",$filename);
			        $filename=str_replace(".cout","",$filename);
				//$filename=str_replace(".","",$filename);

                                //preg_match("/full(.*?).dat/s",$filename,$out);
                                //$filename=$out[0];
                                //print $out;
                                //if (! write_file(APPPATH."/views/data/vandan/september/a.txt", $filename,'w'))
                                //{
				...
				if ( ! write_file(APPPATH."/".$filename.".cout", $ans,'w'))
				{
					 echo 'Unable to write the file';
				}
				else
	...
	public function post_action_save_chain()
	{
	    $this->load->helper('cookie');
	    $this->load->database();
	    $filename=get_cookie('filename');$filename=str_replace("application","",$filename);
            $filename=str_replace("/","",$filename);$filename=str_replace(".","",$filename);

            $message1 = $_POST['chainsend1'];$message2 = $_POST['chainsend2'];$message3 = $_POST['chainsend3'];	   
            
            $chainheadon = $_POST['chainheadon'];$chainheadchainid = $_POST['chainheadchainid'];$chainheadcid=$_POST['chainheadcid'];
		
	    $piecesmessage1 = explode("%$%", $message1);$piecesmessage2 = explode("%$%", $message2);$piecesmessage3 = explode("%$%", $message3);
